...
class Controller:
    ...
    def counter_items(self):
        ...
        if len(self.mac_gyver.inventory) == 1:
            # The text is "inventory : 1" with "##14e0e3" color
            self.dsp.counter_text = \
                self.dsp.items_font.render("inventory : 1",
                                           True, pygame.Color("#14e0e3"))
        # If Macgyver have two item in his inventory
        elif len(self.mac_gyver.inventory) == 2:
            # The text is "inventory : 2" with "##14e0e3" color
            self.dsp.counter_text = \
                self.dsp.items_font.render("inventory: 2",
                                           True, pygame.Color("#14e0e3"))
        # If Macgyver have three item in his inventory
        elif len(self.mac_gyver.inventory) == 3:
            # The text is "inventory : 3" with "##14e0e3" color
            self.dsp.counter_text = \
                self.dsp.items_font.render("inventory : 3",
                                           True, pygame.Color("#14e0e3"))
